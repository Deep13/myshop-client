...
function calcAmount(row) {
    ...
    if (hasDiscount) {
        let discountPerUnit =
            discParsed.type === "percent" ? (mrp * discParsed.value) / 100 : discParsed.value;
...
const findItemByExactCode = (text) => {
    const q = String(text || "").trim().toLowerCase();
    if (!q) return null;
    return ITEM_MASTER.find((it) => String(it.code || "").trim().toLowerCase() === q) || null;
};
                                ...
                                >
                                    {filteredCustomers.map((c) => (
                                        ...
                                        >
                                            ...
                                            <div style={{ fontSize: 12, opacity: 0.75 }}>{c.phone} • {c.type}</div>
                                        </div>
                                    ))}
                                </div>
                            )}
                    ...
                    {rows.map((r, idx) => {
                        ...
                        return (
                            <tr key={idx} className="align-middle">
                                ...
                                <td><Form.Control style={{ border: 0, boxShadow: "none" }} value={r.hsn} onChange={(e) => updateRow(idx, { hsn: e.target.value })} /></td>
                                <td><Form.Control style={{ border: 0, boxShadow: "none" }} value={r.batchNo} onChange={(e) => updateRow(idx, { batchNo: e.target.value })} /></td>
                                <td>
                                    <Form.Control style={{ border: 0, boxShadow: "none" }}
                                        type="date"
                                        value={r.expDate}
                                        onChange={(e) => updateRow(idx, { expDate: e.target.value })}
                                    />
                                </td>
                                <td><Form.Control style={{ border: 0, boxShadow: "none" }} value={r.mrp} onChange={(e) => updateRow(idx, { mrp: e.target.value })} inputMode="decimal" /></td>
                                <td><Form.Control style={{ border: 0, boxShadow: "none" }} value={r.qty} onChange={(e) => updateRow(idx, { qty: e.target.value })} inputMode="numeric" /></td>
                                ...
                                <td><Form.Control style={{ border: 0, boxShadow: "none" }} value={r.tax} onChange={(e) => updateRow(idx, { tax: e.target.value })} inputMode="decimal" /></td>
                                <td><Form.Control style={{ border: 0, boxShadow: "none" }} value={r.amount} disabled /></td>
                            </tr>
                        );
                    })}
                </tbody>
                                ...
                                >
                                    <div>
                                        <Form.Select
                                            value={p.type}
                                            onChange={(e) =>
                                                updatePaymentRow(i, { type: e.target.value })
                                            }
                                        >
                                            <option value="Cash">Cash</option>
                                            <option value="Card">Card</option>
                    ...
                    <div style={{ display: "flex", gap: 12, flexDirection: "column" }}>
                        <div>
                            ...
                            <div style={{ fontSize: 12, opacity: 0.75, marginTop: 4 }}>
                                Discount value: ₹ {billDiscountValue.toFixed(2)}
                            </div>
                        </div>
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            {roundOffEnabled && (
                                <div style={{ fontSize: 12, opacity: 0.75, marginTop: 4 }}>
                                    Round Off: +₹ {roundOffDiff.toFixed(2)}
                                </div>
                            )}
                        </div>
                    </div>
