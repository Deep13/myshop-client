...
<style>
@page{size:${shop.paperWidth} auto;margin:0mm}
*{margin:0;padding:0;box-sizing:border-box}
html{margin:0 !important;padding:0 !important}
body{width:${pw};margin:0 auto;font-family:'Courier New',Courier,monospace;font-size:${is58 ? "11px" : "13px"};color:#000;padding:2px 2px 4px;-webkit-print-color-adjust:exact;print-color-adjust:exact;font-weight:700}
...
.logo svg{width:${is58 ? "32px" : "40px"};height:${is58 ? "32px" : "40px"}}
...
@media print{html,body{margin:0 !important;padding:0 !important}body{width:${pw};padding:2px 2px 4px}}
</style></head>
<body>
...
/**
 * Generate receipt HTML string (same as printReceipt but returns HTML instead of printing).
 * Used by MobileSale to render as an image for download.
 */
export function buildReceiptHTML(data) {
  const shop = getShopSettings();
  const is58 = shop.paperWidth === "58mm";
  const pw = is58 ? "180px" : "280px"; // pixel widths for screen rendering

  const time = data.invoiceTime || new Date().toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit", hour12: true });

  let taxableTotal = 0, totalTax = 0, totalSavings = 0;
  for (const item of data.items) {
    const amt = Number(item.amount || 0);
    const taxPct = Number(item.tax || 0);
    if (taxPct > 0) {
    ...
    const mrp = Number(item.mrp || 0);
    const sp = Number(item.price || 0);
    if (mrp > sp && sp > 0) totalSavings += (mrp - sp) * Number(item.qty || 0);
  }
  taxableTotal = Math.round(taxableTotal * 100) / 100;
  totalTax = Math.round(totalTax * 100) / 100;
  totalSavings = Math.round(totalSavings * 100) / 100;
...
<td class="v-top r b" style="padding:2px 4px">${n(item.amount)}</td>
  ...
  sRows.push(["Total", n(data.total)]);
  sRows.push(["Received", n(data.received)]);
  if (Number(data.balance || 0) > 0) sRows.push(["Balance", n(data.balance)]);
...
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{width:${pw};margin:0;font-family:'Courier New',Courier,monospace;font-size:${is58 ? "11px" : "13px"};color:#000;padding:8px 4px;font-weight:700;background:#fff}
table{width:100%;border-collapse:collapse}
th{font-weight:900;padding:2px 0}
td{font-weight:700}
.b{font-weight:900}
